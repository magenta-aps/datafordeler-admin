...
class ViewsTestCase(TestCase):
    ...
    def test_frontpage_not_logged_in(self):
        resp = self.client.get('/frontpage/')
    ...
    def test_login_user_password(self):
        resp = self.client.post('/login/', {'username': 'jakob@data.nanoq.gl', 'password': 'jacob'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(resp.context['user'].is_authenticated())

    def test_login_user_password_fail(self):
        resp = self.client.post('/login/', {'username': 'jako@data.nanoq.gl', 'password': 'jacob'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertFalse(resp.context['user'].is_authenticated())

    def test_rst_doc_view(self):
        resp = self.client.get('/doc/access-rights.rst')
        self.assertEqual(resp.status_code, 200)